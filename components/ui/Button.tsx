...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variants = {
    ...
    outline: "border border-accent hover:bg-accent text-white bg-transparent",
    ghost: "hover:bg-accent text-muted hover:text-white",
  ...
  return (
    <Component
      className={`${baseStyles} ${variants[variant]} ${sizes[size]} ${className} ${
        Component !== 'button' && isDisabled ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''
      }`}
